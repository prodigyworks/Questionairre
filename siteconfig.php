...
<?php
	if (isset($_POST['domainurl'])) {
		...
		$paypalnotifieremail = mysql_escape_string($_POST['paypalnotifieremail']);
		$paypalfacilitatoremail = mysql_escape_string($_POST['paypalfacilitatoremail']);
		$paypalsandbox = (isset($_POST['paypalsandbox']) && $_POST['paypalsandbox'] == "on") ? 1 : 0;
		...
		$qry = "UPDATE {$_SESSION['DB_PREFIX']}siteconfig SET 
				...
				paypalnotifieremail = '$paypalnotifieremail',
				paypalfacilitatoremail = '$paypalfacilitatoremail',
...
<form id="contentForm" name="contentForm" method="post" class="entryform">
	...
	<label>Paypal Notification Email</label>
	<input required="true" type="text" class="textbox90" id="paypalnotifieremail" name="paypalnotifieremail" value="<?php echo $member['paypalnotifieremail']; ?>" />

	<label>Paypal Facilitator Email</label>
	<input required="true" type="text" class="textbox90" id="paypalfacilitatoremail" name="paypalfacilitatoremail" value="<?php echo $member['paypalfacilitatoremail']; ?>" />

	<label>Paypal Sandbox Email</label>
	<input required="true" type="checkbox" id="paypalsandbox" name="paypalsandbox" <?php if ($member['paypalsandbox'] == 1) echo "checked"; ?> />
