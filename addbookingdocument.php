<?php
	...
	$qry = "SELECT A.id FROM {$_SESSION['DB_PREFIX']}documents A " .
		   "WHERE A.sessionid = '" . session_id() . "' " .
		   	"AND A.id NOT IN (SELECT documentid FROM {$_SESSION['DB_PREFIX']}bookingdocs WHERE documentid = A.id) " .
		   "ORDER BY A.id";
	...
	while (($member = mysql_fetch_assoc($result))) {
		$qry = "INSERT INTO {$_SESSION['DB_PREFIX']}bookingdocs " .
				"(bookingid, documentid, createddate, metacreateddate, metacreateduserid, metamodifieddate, metamodifieduserid) " .
				"VALUES " .
				"($bookingid, " . $member['id'] . ", NOW(), NOW(), $memberid, NOW(), $memberid)";
