...
class SiteConfigClass {
	...
	public $paypalnotifieremail;
	public $paypalfacilitatoremail;
}
		...
		if (! isset($_SESSION['SITE_CONFIG'])) {
			...
			if ($result) {
				if (mysql_num_rows($result) == 1) {
					...
					$data->paypalsandbox = $member['paypalsandbox'];
					$data->paypalnotifieremail = $member['paypalnotifieremail'];
					$data->paypalfacilitatoremail = $member['paypalfacilitatoremail'];
...
function smtpmailer($to, $from, $from_name, $subject, $body, $attachments = array()) { 
	...
	try {
		...
		$mail->SetFrom("info@prodigyworks.co.uk", $from_name);
		$mail->IsHTML(true);
...
function sendRoleMessage($role, $subject, $message, $attachments = array()) {
	...
	if($result) {
		while (($member = mysql_fetch_assoc($result))) {
			smtpmailer($member['email'], 'info@prodigyworks.co.uk', 'Company Name', $subject, getEmailHeader() . "<h4>Dear " . $member['firstname'] . ",</h4><p>" . $message . "</p>" . getEmailFooter(), $attachments);
...
function sendTeamMessage($id, $subject, $message, $footer = "") {
	...
	if($result) {
		while (($member = mysql_fetch_assoc($result))) {
			smtpmailer($member['email'], 'info@prodigyworks.co.uk', 'Company Name', $subject, getEmailHeader() . "<h4>Dear " . $member['firstname'] . ",</h4><p>" . $message . "</p>" . getEmailFooter(). $footer);
...
function sendUserMessage($id, $subject, $message, $footer = "", $attachments = array(), $action = "") {
	...
	if($result) {
		while (($member = mysql_fetch_assoc($result))) {
			smtpmailer($member['email'], 'info@prodigyworks.co.uk', 'Company Name', $subject, getEmailHeader() . "<h4>Dear " . $member['firstname'] . ",</h4><p>" . $message . "</p>" . getEmailFooter(). $footer, $attachments);
...
function sendCourtMessage($id, $subject, $message, $footer = "", $attachments = array(), $action = "") {
	...
	if($result) {
		while (($member = mysql_fetch_assoc($result))) {
			smtpmailer($member['email'], 'info@prodigyworks.co.uk', 'Company Name', $subject, getEmailHeader() . "<h4>Dear " . $member['name'] . ",</h4><p>" . $message . "</p>" . getEmailFooter(). $footer, $attachments);
